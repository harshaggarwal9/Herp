...
export const createStudent = async (req, res) => {
  ...
  try {
    ...
    if (!parent)
      res.status(404).json({ message: "parent not exist first create parent" });
    const classData = await classModel.findOne({ className, section });
    if (!classData)
      res.status.json({ message: "class not exist first create class" });
    ...
    res
      .status(201)
      .json({ student: studentData, message: "student created successfully" });
  } catch (error) {
    console.log(error);
...
export const getStudent = async (req, res) => {
  const { id } = req.params;
  try {
    const user = await Student.findOne({ userId: id }).populate([
      { path: "userId" },
      { path: "classId" },
      {
        path: "parent",
...
export const deleteStudent = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Student deleted successfully", StudentById });
  } catch (error) {
